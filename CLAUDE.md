...
- **Código do Dagster carrega de servidor dedicado (`dagster-codigo`), nunca de `python_module`.** Code server efêmero tem heartbeat: perdeu, se desliga e leva junto os runs que executavam dentro dele — que ficam `STARTED` para sempre e entopem a fila de 10 do `QueuedRunCoordinator`. Sintoma: tudo "no ar" no `docker compose ps` e `/fontes/pipeline` dizendo `vivo: false`. `run_monitoring` sozinho não resolve (para enterrar o run, o monitor precisa perguntar ao servidor que morreu). Derrubou a coleta em 07/08 (6 h) e 08/08 (38 min). Procedimento de desentupimento em `deploy/README.md`.
- **Número que veio do snapshot não envelhece.** O ticker anunciava "19/21 fontes online" com a ingestão morta havia 40 min, porque lia `contadores` do último snapshot; o rodapé, que lê `/fontes` com checagem de frescor, dizia 7 de 21 na mesma tela. Contagem de estado vem sempre de `/fontes`.
